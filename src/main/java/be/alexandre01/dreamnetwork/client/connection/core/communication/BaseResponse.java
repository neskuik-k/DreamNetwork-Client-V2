...
public class BaseResponse extends CoreResponse {
    ...
    public void onResponse(Message message, ChannelHandlerContext ctx, IClient client) throws Exception {
        ...
        IDNChannel dnChannel = this.client.getChannelManager().getChannel(message.getChannel());
        if(dnChannel != null){
        ...
        if(message.getHeader()!=null){
            if(message.getHeader().equals("cData") && message.getChannel() != null){
                if(this.client.getChannelManager().getClientsRegistered().containsKey(message.getChannel())){
                    IDNChannel channel = this.client.getChannelManager().getChannel(message.getChannel());
            ...
            if(message.getHeader().equals("channel") && message.getChannel() != null){
                if(this.client.getChannelManager().getClientsRegistered().containsKey(message.getChannel())){
                    final Collection<IClient> clients = this.client.getChannelManager().getClientsRegistered().get(message.getChannel());
                    if(!clients.isEmpty()){
                        boolean resend = true;
                        ...
                        for(IClient c : this.client.getChannelManager().getClientsRegistered().get(message.getChannel())){
                            if(!resend && c == client){
                                continue;
                            }
